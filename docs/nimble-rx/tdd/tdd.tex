...
\begin{itemize}
  \item \textbf{Data collection.} Data is collected as \texttt{LeRobotDataset} episodes, where each episode captures an instruction, a camera frame, and the corresponding 6-DOF grasp/place action chunk. The initial dataset comprises 10 episodes per item type, scaling toward $\geq$50 episodes per item type. Episodes themselves will be collected via human teleoperation on a keyboard for now.
